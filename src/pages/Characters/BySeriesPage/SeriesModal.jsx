...
const SeriesModal = ({ series, isOpen, onClick }) => {
  ...
  return (
    <Modal width={600} title={series.name} open={isOpen} onOk={()=> onClick()}> 
      <div className="w-full flex gap-4">
        <img className="w-[200px] object-cover"
          src={`${series.thumbnail.path}/portrait_uncanny.${series.thumbnail.extension}`}/>
        <div className="flex-1 flex flex-col gap-6">
          <h1 className="text-2xl font-montserrat"> {series.title} </h1>
          <p className="text-lg font-roboto"> {series.description} </p>
        </div>
      </div>
      {
        pagination && (
          <div className="w-full flex flex-col gap-2 items-center">
            ...
            <div className="w-full overflow-x-auto flex gap-4">
...

              {
                ...
                <button onClick={() => fetchComics(series.id, pagination.offset + pagination.limit)} 
