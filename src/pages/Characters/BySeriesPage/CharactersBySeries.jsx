...
import {findseriesByName } from "../../../services/CatalogsService";
import { toast } from "react-toastify";
import { useLoadingContext } from "../../../contexts/LoadingContext";
import SeriesLayout from "./SeriesLayout";


const CharactersBySeriesPage = () => {
  ...
  const onFinish = async (values) => {
    ...
    const _data = await findseriesByName(values.search, 0, 20);
  ...
  const onMore = async () => {
    ...
    if(pagination.offset + pagination.limit <= pagination.total) {
      ...
      const _data = await findseriesByName(search, _offset, 5);
  ...
  return(
    <PrivateRoute>
      ...
      {
        pagination && <SeriesLayout pagination={pagination} items={items} onMore={onMore}/>
      }
    </PrivateRoute>
    );
}

export default CharactersBySeriesPage;